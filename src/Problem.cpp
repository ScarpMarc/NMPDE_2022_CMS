...
Problem::setup()
{
  // Create the mesh.
  {
    ...
    //  "../mesh/" + filename + ".msh";               // seleziono il nome file

    std::ifstream grid_in_file(simulation_settings.file_name);     // carico il file
    grid_in.read_msh(grid_in_file);                 // leggo il file
...
Problem::solveLinearSystem()
{
  ...
  */
  ...
  SolverControl solver_control(simulation_settings.max_solver_iteration_amt, desired_solver_precision * residual_vector.l2_norm());
...
{
  // parametri
  const unsigned int n_max_iters        = max_newton_iteration_amt;
  const double       residual_tolerance = desired_newton_precision;
